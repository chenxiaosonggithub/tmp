...
{
    node->refcount++;
    std::cout << "ACQUIRE " << std::hex << node << std::dec
               << " (" << node->name << ") rc=" << node->refcount << std::endl;
}
...
{
    std::cout << "RELEASE " << std::hex << node << std::dec
               << " (" << node->name << ") rc=" << node->refcount << std::endl;
    if (node->refcount > 0) {
        node->refcount--;
        if (!node->refcount) {
        ...
        std::cerr << std::hex << node << std::dec << " refcount=0" << std::endl;
    }
}
        ...
        struct node *parent, const char *name, const char* actual_name)
{
    ...
    if (fuse->global->inode_ctr == 0) {
        std::cerr << "No more inode numbers available" << std::endl;
        return NULL;
    }
...
static void fuse_status(struct fuse *fuse, __u64 unique, int err)
{
    ...
    if (ret == -1) {
        ...
        std::cerr << "*** STATUS FAILED: written " << ret << " expected "
                   << sizeof(hdr) << " ***" << std::endl;
    }
}
...
static void fuse_reply(struct fuse *fuse, __u64 unique, void *data, int len)
{
    ...
    if (ret == -1) {
        std::cerr << "*** REPLY FAILED ***" << std::endl;
    } else if (static_cast<size_t>(ret) != sizeof(hdr) + len) {
        std::cerr << "*** REPLY FAILED: written " << ret << " expected "
                   << sizeof(hdr) + len << " ***" << std::endl;
    }
}
        ...
        const char* path)
{
    ...
    struct stat s;
     ...
     std::cerr << "[" << __func__ << "]actual_name=" << actual_name << ",path=" << path << std::endl;
    if (lstat(path, &s) == -1) {
        return -errno;
    }
        ...
        const char* path)
{
    ...
    struct stat s;
     ...
     std::cerr << "[" << __func__ << "][" << __LINE__ << "]path=" << path << std::endl;
    if (lstat(path, &s) == -1) {
        return -errno;
    }
    memset(&out, 0, sizeof(out));
...
static void fuse_notify_delete(struct fuse* fuse, const __u64 parent,
    ...
    if (ret == -1) {
        ...
        std::cerr << "*** NOTIFY FAILED: written " << ret << " expected "
                   << sizeof(hdr) + sizeof(data) + namelen + 1 << " ***" << std::endl;
    }
}
        ...
        const struct fuse_in_header *hdr, const char* name)
{
    ...
    parent_node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid,
     ...
     std::cerr << "[" << handler->token << "] LOOKUP " << name << " @ " << hdr->nodeid
               << " (" << (parent_node ? parent_node->name : "?") << ")" << std::endl;
    pthread_mutex_unlock(&fuse->global->lock);
        ...
        const struct fuse_in_header *hdr, const struct fuse_forget_in *req)
{
    ...
    node = lookup_node_by_id_locked(fuse, hdr->nodeid);
     std::cerr << "[" << handler->token << "] FORGET #" << req->nlookup
               << " @ " << std::hex << hdr->nodeid
        ...
        const struct fuse_in_header *hdr, const struct fuse_getattr_in *req)
{
    ...
    std::cerr << "[" << handler->token << "] GETATTR flags=" << req->getattr_flags
        ...
        const struct fuse_in_header *hdr, const struct fuse_setattr_in *req)
{
    ...
    pthread_mutex_lock(&fuse->global->lock);
    node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid, path, sizeof(path));
     std::cerr << "[" << handler->token << "] SETATTR fh=" << std::hex << req->fh
        ...
        const struct fuse_in_header* hdr, const struct fuse_mknod_in* req, const char* name)
{
    ...
    parent_node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid,
            parent_path, sizeof(parent_path));
     std::cerr << "[" << handler->token << "] MKNOD " << name << " 0" << std::oct << req->mode
               << " @ " << std::hex << hdr->nodeid
               << " (" << (parent_node ? parent_node->name : "?") << ")" << std::endl;
    pthread_mutex_unlock(&fuse->global->lock);
        ...
        const struct fuse_in_header* hdr, const struct fuse_mkdir_in* req, const char* name)
{
    ...
    parent_node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid,
            parent_path, sizeof(parent_path));
     std::cerr << "[" << handler->token << "] MKDIR " << name << " 0" << std::oct << req->mode
               << " @ " << std::hex << hdr->nodeid
               << " (" << (parent_node ? parent_node->name : "?") << ")" << std::endl;
    pthread_mutex_unlock(&fuse->global->lock);
        ...
        const struct fuse_in_header* hdr, const char* name)
{
    ...
    parent_node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid,
            parent_path, sizeof(parent_path));
     std::cerr << "[" << handler->token << "] UNLINK " << name << " @ " << std::hex << hdr->nodeid
               << " (" << (parent_node ? parent_node->name : "?") << ")" << std::endl;
    pthread_mutex_unlock(&fuse->global->lock);
        ...
        const char* old_name, const char* new_name)
{
    ...
    new_parent_node = lookup_node_and_path_by_id_locked(fuse, req->newdir,
     ...
     std::cerr << "[" << handler->token << "] RENAME " << old_name << "->" << new_name
               << " @ " << std::hex << hdr->nodeid
               << " (" << (old_parent_node ? old_parent_node->name : "?") << ") -> "
               << std::hex << req->newdir
               << " (" << (new_parent_node ? new_parent_node->name : "?") << ")" << std::endl;
        ...
        const struct fuse_in_header* hdr, const struct fuse_open_in* req)
{
    ...
    pthread_mutex_lock(&fuse->global->lock);
    node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid, path, sizeof(path));
     std::cerr << "[" << handler->token << "] OPEN 0" << std::oct << req->flags
        ...
        const struct fuse_in_header* hdr, const struct fuse_read_in* req)
{
     ...
     * saves us 128KB per request handler thread at the cost of this scary comment. */
     ...
     std::cerr << "[" << handler->token << "] READ " << std::hex << h << std::dec
               << "(" << h->fd << ") " << size << "@" << offset << std::endl;
    if (size > MAX_READ) {
        return -EINVAL;
    }
        ...
        const void* buffer)
{
    ...
    if (req->flags & O_DIRECT) {
     ...
     std::cerr << "[" << handler->token << "] WRITE " << std::hex << h << std::dec
               << "(" << h->fd << ") " << req->size << "@" << req->offset << std::endl;
        ...
        const struct fuse_in_header* hdr)
{
    ...
    pthread_mutex_lock(&fuse->global->lock);
     std::cerr << "[" << handler->token << "] STATFS" << std::endl;
        ...
        const struct fuse_in_header* hdr, const struct fuse_release_in* req)
{
    struct handle *h = static_cast<struct handle*>(id_to_ptr(req->fh));
     ...
     std::cerr << "[" << handler->token << "] RELEASE " << std::hex << h << std::dec
               << "(" << h->fd << ")" << std::endl;
    close(h->fd);
    free(h);
    return 0;
        ...
        const struct fuse_in_header* hdr, const struct fuse_fsync_in* req)
{
    ...
    if (is_dir) {
     ...
     std::cerr << "[" << handler->token << "] " << (is_dir ? "FSYNCDIR" : "FSYNC") << " "
               << std::hex << req->fh << std::dec << "(" << fd << ") is_data_sync=" << is_data_sync << std::endl;
...
static int handle_flush(struct fuse* fuse, struct fuse_handler* handler,
        const struct fuse_in_header* hdr)
{
    std::cerr << "[" << handler->token << "] FLUSH" << std::endl;
    return 0;
}
        ...
        const struct fuse_in_header* hdr, const struct fuse_open_in* req)
{
    ...
    pthread_mutex_lock(&fuse->global->lock);
    node = lookup_node_and_path_by_id_locked(fuse, hdr->nodeid, path, sizeof(path));
     std::cerr << "[" << handler->token << "] OPENDIR @ " << std::hex << hdr->nodeid
               << " (" << (node ? node->name : "?") << ")" << std::endl;
    pthread_mutex_unlock(&fuse->global->lock);
        ...
        const struct fuse_in_header* hdr, const struct fuse_read_in* req)
{
    ...
    if (req->offset == 0) {
        ...
        std::cerr << "[" << handler->token << "] calling rewinddir()" << std::endl;
        rewinddir(h->d);
    }
    de = readdir(h->d);
    if (!de) {
        ...
        const struct fuse_in_header* hdr, const struct fuse_init_in* req)
{
    ...
    if (req->major != FUSE_KERNEL_VERSION || req->minor < 6) {
        std::cerr << "Fuse kernel version mismatch: Kernel version "
                   << req->major << "." << req->minor
                   << ", Expected at least " << FUSE_KERNEL_VERSION << ".6" << std::endl;
        return -1;
    }
...
//    case FUSE_REMOVEXATTR:
    ...
    default: {
        std::cerr << "[" << handler->token << "] NOTIMPL op=" << hdr->opcode
                   << "uniq=" << std::hex << hdr->unique << "nid=" << hdr->nodeid << std::dec << std::endl;
        return -ENOSYS;
    }
    }
}
...
void handle_fuse_requests(struct fuse_handler* handler)
{
    ...
    for (;;) {
        ...
        if (len == -1) {
            if (errno == ENODEV) {
                std::cerr << "[" << handler->token << "] someone stole our marbles!" << std::endl;
                exit(2);
            }
            std::cerr << "[" << handler->token << "] handle_fuse_requests: " << strerror(errno) << std::endl;
            continue;
        }

        if (static_cast<size_t>(len) < sizeof(struct fuse_in_header)) {
            std::cerr << "[" << handler->token << "] request too short: len=" << len << std::endl;
            continue;
        }
        ...
        if (hdr->len != static_cast<size_t>(len)) {
            std::cerr << "[" << handler->token << "] malformed header: len=" << len
                       << ", hdr->len=" << hdr->len << std::endl;
            continue;
        }

        std::cerr << "[" << __func__ << "] opcode=" << hdr->opcode << std::endl;
        ...
        std::cerr << "[" << __func__ << "] res=" << res << std::endl;
