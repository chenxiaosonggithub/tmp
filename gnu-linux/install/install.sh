...
fedora_docker()
{
	...
	sudo dnf -y install ncurses-devel clang llvm flex bison bc kmod pahole lld ccache openssl-devel openssl git-email b4
	sudo dnf -y install binutils-aarch64-linux-gnu gcc-aarch64-linux-gnu
...
ubuntu_docker()
{
	apt-get update -y
	apt install -y sudo
	sudo apt install -y qemu-system vim git
	sudo apt install -y build-essential flex bison bc kmod pahole libelf-dev libssl-dev libncurses-dev zstd # 内核编译
